...
void cudaProcess(simConnect::simData payload) {
  ...
  /* Pre fetch to ease page faults */
  cudaSuccess(cudaGetDevice(&device));
  printf("Returned device %d\n", device);
  //cudaSuccess(cudaMemPrefetchAsync(d_processedData, sizeof(processedData_t), device, NULL));
...
void *gpuWorker(void *params) {
    ...
    /* Fill in the proto object */
    convertedData.set_sztitle(toStr);
    convertedData.set_dabsolutetime(63744429559.83358);
    convertedData.set_dtime(64759.83203125);
    convertedData.set_usimonground(1.0000000);
    convertedData.set_daltitude(92.940918180555627);
    convertedData.set_dheading(3.4027943720320466);
    convertedData.set_dspeed(110.96809387207031);
    convertedData.set_dverticalspeed(466.78314208984375);
    convertedData.set_dgpseta(0);
    convertedData.set_dlatitude(30.46249819919468);
    convertedData.set_dlongitude(-86.523398558488353);
    convertedData.set_dsimtime(3028.9320206769653);
    convertedData.set_dtemperature(-9.1166267395019531);
    convertedData.set_dairpressure(636.3985595703125);
